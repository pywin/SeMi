...
class MemoryBank_cost(nn.Module):
    def __init__(self, classes=10, num_feature_per_class=64, feature_dim=128, use_cuda=True):
        super(MemoryBank_cost, self).__init__()
        self.classes = classes
        self.num_feature_per_class = num_feature_per_class
        self.feature_dim = feature_dim
    ...
    def get_sup(self, labeled_target, unlabeled_target, selected_mask, num_samples=128):
        ...
        labels = torch.argmax(labels, dim=1)
        total_samples = torch.tensor(labels.size(0) + num_samples).cuda()
        labeled_class_counts = torch.zeros(self.classes).cuda()
        ...
        target_counts = torch.ones(self.classes).cuda()*base_count
        te1 = time.time()
        print(f'm1_cost:{te1-ts1}')
        ts2 = time.time()
        ...
        #print(class_counts_sum.item())
        supple_features = torch.zeros(class_counts_sum, self.memory.size(2)).cuda()
        #print(selected_features.size())
        supple_labels = torch.zeros(class_counts_sum).cuda()
        n = 0
        for class_idx in range(self.memory.size(0)):
            ...
            if class_num_samples == 0:
                continue
            # 从当前 class 中采样 num_samples 个特征
            #print(self.memory.size(1))
            print(class_num_samples)
            #sampled_indices = torch.multinomial(torch.ones(self.memory.size(1)), class_num_samples, replacement=False)
            ...
            supple_labels[n:n+class_num_samples] = torch.tensor([class_idx] * class_num_samples).cuda()
            n += class_num_samples
        # 合并成一个二维张量
        te2 = time.time()
        print(f'm2_cost:{te2-ts2}')
        #supple_features = torch.cat(selected_features, dim=0)
        #supple_labels = torch.tensor(selected_labels)
        #supple_labels = torch.eye(self.classes)[supple_labels]
