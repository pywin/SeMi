...
from utils_sup import make_imb_data, save_checkpoint, FixMatch_Loss, Balanced_FixMatch_Loss, CenterLoss, SupervisedContrastiveLoss, MemoryBank, ProtoClassifier, update_qbar, probs_monitor, get_labeled_class_weights, probs_monitor_
...
def main():
    ...
    parser.add_argument('--conswt', default=0.15, type=float,
                        help='')
    parser.add_argument('--proto_t', default=0.1, type=float,
                        help='')
    parser.add_argument('--scale', default=0.75, type=float,
                        help='')
    parser.add_argument('--proto_dim', default=128, type=int,
                        help='')
    parser.add_argument('--mb_size', default=128, type=int,
                        help='')
    parser.add_argument('--twarm', default=1, type=int,
                        help='')
    parser.add_argument('--use_pc', default=1, type=int,
                        help='')
    parser.add_argument('--decay_step', type=int, default=50,
                        help='update the memory bank step')
    parser.add_argument('--df', default=0.99, type=float, help='decay factor for class weights')
    parser.add_argument('--tau_tar', default=0.1, type=float)
    parser.add_argument('--alpha', default=0.0, type=float)
    parser.add_argument('--opt', default='sgd', type=str)
    parser.add_argument('--pula', default=0, type=int)
    ...
    if args.dataset != 'smallimagenet':
        dataset_root = 'datasets/'+args.dataset_name
    else:
        if args.img_size == 32:
            dataset_root = '/home/wzx/baseline/CoSSL/Imagenet-32-processed'
        else:
            dataset_root = '/home/wzx/baseline/CoSSL/Imagenet-64-processed'
    ...
    memory = MemoryBank(classes=args.num_classes, num_feature_per_class=args.mb_size, device=args.device)
    #memory_w = MemoryBank(classes=num_class, num_feature_per_class=args.mb_size)
    pc = ProtoClassifier(num_classes=args.num_classes)
    N_SAMPLES_PER_CLASS = make_imb_data(args.num_max, args.num_classes, args.imb_ratio_label)
    unlabeled_class_weights = torch.ones(args.num_classes).to(args.device)
    unlabeled_class_weights_inv = torch.ones(args.num_classes).to(args.device)
    unlabeled_class_weights_inv_base = torch.ones(args.num_classes).to(args.device)
    labeled_class_weights = get_labeled_class_weights(N_SAMPLES_PER_CLASS)
    labeled_class_weights = torch.tensor(labeled_class_weights).to(args.device)
    print(f'\nLabeled_class_weights:{labeled_class_weights}')

    model.zero_grad()
    train(args, labeled_trainloader, unlabeled_trainloader, test_loader, model, optimizer, ema_model, scheduler, memory, pc, unlabeled_class_weights, unlabeled_class_weights_inv_base, labeled_class_weights)
    args.logger.close()


def train(args, labeled_trainloader, unlabeled_trainloader, test_loader, model, optimizer, ema_model, scheduler, memory, pc, unlabeled_class_weights, unlabeled_class_weights_inv_base, labeled_class_weights):
    global best_acc
    global best_acc_b
    epsilon = 1e-8
    qbar = torch.zeros(1, args.proto_dim).to(args.device)
    label_counts = torch.zeros(args.num_classes).to(args.device)
    test_accs = []
    avg_time = []
    end = time.time()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for batch_idx in range(args.eval_step):
            ...
            feat_x = logits_feat[:batch_size]
            feat_u_w = logits_feat[batch_size:2*batch_size]
            feat_u_s = logits_feat[2*batch_size:3*batch_size]
            feat_u_s1 = logits_feat[3*batch_size:4*batch_size]
            feat_u_ss = logits_feat[2*batch_size:4*batch_size]
            ...
            #print(pseudo_label)
            # pc area
            if args.use_pc and epoch > args.est_epoch:
                protos = memory.get_protos()
                targets_pc = pc(feat_u_s.squeeze(), protos, T=args.tau_tar, norm=False)
                pseudo_label = (1 - args.alpha*(epoch/args.eval_step)*(unlabeled_class_weights_inv_base)) * pseudo_label + args.alpha*(epoch/args.eval_step*(unlabeled_class_weights_inv_base))*targets_pc
            #print('*'*50)
            #print(pseudo_label)
            ...
            pseudo_label_twice = torch.cat([pseudo_label, pseudo_label], dim=0)
            #print(pseudo_label_twice)
            ...
            u_confidences = torch.cat([max_probs, max_probs])
            ...
            mask_twice_ss_b_h2 = torch.cat([mask_ss_b_h2, mask_ss_b_h2], dim=0).to(args.device)
            mask_twice_ss_t = torch.cat([mask_ss_t, mask_ss_t], dim=0).to(args.device)

            logits_u_s_twice = torch.cat([logits_u_s, logits_u_s1], dim=0).to(args.device)
            targets_u_twice = torch.cat([targets_u, targets_u], dim=0).to(args.device)
            targets_u_h2_twice = torch.cat([targets_u_h2, targets_u_h2], dim=0).to(args.device)

            logits_u_s_b_twice = torch.cat([logits_u_s_b, logits_u_s1_b], dim=0).to(args.device)
            ...
            if batch_idx == args.eval_step -1:
                unlabeled_class_weights, unlabeled_class_weights_inv, unlabeled_class_weights_inv_base = probs_monitor_(pseudo_label, label_counts, mask_ss_t, epoch, args.num_classes, decay_factor=args.df, return_probs=True)
                print('*'*100 + '\n', unlabeled_class_weights)
            else:
                label_counts = probs_monitor_(pseudo_label, label_counts, mask_ss_t, epoch, args.num_classes, decay_factor=args.df, return_probs=False)
            #print(pseudo_label_twice)

            memory_bank = memory.push(feat_u_ss.detach().clone(), pseudo_label_twice, u_confidences, mask_twice_ss_t)

            if epoch > args.est_epoch:
                # ohem area
                probabilities = torch.clamp(pseudo_label, epsilon, 1.0 - epsilon)
                ohem_weights = -torch.sum(probabilities * torch.log2(probabilities), dim=1) / torch.log2(torch.tensor(args.num_classes, dtype=torch.float32)) * args.scale + (1 - args.scale)
                ohem_weights = torch.cat([ohem_weights, ohem_weights])
                # feature align
                q_u_w = torch.cat([feat_u_w, feat_u_w], dim=0)
                q_u_s = torch.cat([feat_u_s, feat_u_s1], dim=0)
                q_out = F.softmax((q_u_w.detach() - qbar) / (args.proto_t/5), dim=1)
                inv_mask = mask_twice_ss_t.bool().logical_not().float()
                Lcons = (torch.sum(-q_out * F.log_softmax(q_u_s / args.proto_t, dim=1), dim=1) * inv_mask).mean()
                qbar = update_qbar(qbar, q_out)
            else:
                ohem_weights = torch.ones((2*batch_size)).to(args.device)
                Lcons = torch.tensor(0.0).to(args.device)

            Lu = (F.cross_entropy(logits_u_s_twice, targets_u_twice,
                                  reduction='none') * mask_twice_ss_t * ohem_weights).mean()
            Lu_b = (F.cross_entropy(logits_u_s_b_twice, targets_u_h2_twice,
                                    reduction='none') * mask_twice_ss_b_h2).mean()
            loss = Lx + Lu + Lx_b + Lu_b + args.conswt*Lcons
            loss.backward()
            ...
            if (batch_idx+1) % args.decay_step == 0:
                #print(u_confidences)
                #print('*'*50 + 'decay' + '*'*50)
                memory.decay()
                memory.reset_em()
